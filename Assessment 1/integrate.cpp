...
double integrate_parallel(double a, double b, long n)
{
    ...
    // Endpoint terms contribute exactly once; kept outside the parallel region
    // so they are never double-counted regardless of thread count.
    double sum = 0.5 * (f(a) + f(b));
    // schedule(guided) outperformed static and dynamic,64 at 128T on CX3 Rome
    // (0.0446 s vs 0.0566 s static, 0.0987 s dynamic,64). Guided tapers chunk
    // sizes to distribute the f(x) spike region without dispatcher contention.
