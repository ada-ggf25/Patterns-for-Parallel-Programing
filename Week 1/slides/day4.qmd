...
Wrap the timed region with a *warm-up* region so the runtime startup doesn't count:

{{< include _partials/day4/omp_get_wtime_timing__time_kernel_warmup.qmd >}}

- The first parallel region pays a one-time team-creation cost (~µs); without warm-up, your first measurement can be 5–10× slower.
- Throw away the warm-up's wall-clock and only `omp_get_wtime` *after* the warm-up.
- The warm-up loop should touch the same memory the timed loop will, so caches and NUMA placement are also warm.

## Timing — pairing two timings in a struct

{{< include _partials/day4/omp_get_wtime_timing__compare_struct.qmd >}}

- Returning a struct (rather than a single ratio) preserves both raw numbers — the caller can compute ratio, speedup, or absolute difference as the analysis demands.
- The two field names (`fast_s`, `slow_s`) are conventional but not enforced — the harness on the next slide labels by *position*, not by which actually turned out faster.

## Timing — the comparison driver

{{< include _partials/day4/omp_get_wtime_timing__compare_function.qmd >}}

- Both implementations route through the same `time_kernel_seconds` so they pay the same warm-up cost and the same timed-region overhead — the two timings are directly comparable.
- For noisy measurements, run each side multiple times and take the minimum (next slide).

## The warm-up trick

```cpp
// Warm-up: pay first-region runtime startup once, throw the result away.
#pragma omp parallel
{ /* trivial work */ }

// Now measure the *steady-state* parallel region.
...
Without warm-up, your first measurement carries the team-creation cost (~µs) and can be 5–10× slower than the second. For a hyperfine harness, `--warmup 1` does the same job.

## Comparing two implementations

```cpp
// Run each k times, take the minimum.
auto bench = [](auto fn) {
    double best = 1e30;
...
const double t_a = bench(impl_a);
const double t_b = bench(impl_b);
const double ratio = t_b / t_a;        // > 1 if B is slower
```

Min-of-k beats single-run on noisy hardware (queue contention, cron jobs, …).
...
For low-OI kernels (A3 Jacobi, STREAM, dense matrix-vector):
...
- **Honest ceiling**. Using Copy's 337 GB/s would inflate the ceiling and unfairly penalise A3 students for a hardware quirk that has nothing to do with their kernel quality.
...
```cpp
struct Counter { double v; };       // 8 B
Counter buckets[N_THREADS];

#pragma omp parallel
{
    int tid = omp_get_thread_num();
    for (int i = 0; i < BIG; ++i)
        buckets[tid].v += 1.0;       // writing my bucket only
}
```
...
## Live demo: false sharing on Rome {visibility="uncounted"}

Live demo (instructor): two runs of the false-sharing harness at 64 threads on a Rome login node:

- `bad_accum`: threads write into a plain `double[]` — eight per cache line, with cross-thread contention.
- `good_accum`: threads write `alignas(64)` `Padded` accumulators.
...
## Tile / blocking — preview

For memory-bound kernels (A3), *tiling* improves cache reuse:

```cpp
// without tiling: streams the whole grid through cache N times
for (size_t step = 0; step < NSTEPS; ++step)
    for (size_t i = 0; i < NX; ++i)
        for (size_t j = 0; j < NY; ++j) ...

// with tiling: process small blocks; reuse them across timesteps
for (size_t step = 0; step < NSTEPS; ++step)
...
Day-5 polish territory. Not required for A3-core; rewarding for the A3 extension if you go that route.
...
- `aligned(x, y, r : 64)` — promise to the compiler that `x`, `y`, `r` are 64-byte-aligned. Compiler emits aligned loads/stores (faster). **Misalign at runtime → segfault.**
- `safelen(8)` — assert at least 8 consecutive iterations are dependence-free. Lets the compiler fold loop unrolling + vectorisation.

Use both when you can; they enable the strongest vectorisation.
...
- `-Rpass-missed=loop-vectorize` — show what *didn't*, with the reason. Often actionable: change pointer types, add `safelen`, fix aliasing.
...
- `simd` — annotate the innermost loop with `simd` + `aligned` + `safelen`; measure.
...
The grader checks internal consistency: `delta_percent` must be consistent with `before_time_s` and `after_time_s` within ±10 %. Canonical timings are *not* compared against your reported numbers — your own self-benchmark is the basis. The grader's separate **reproducibility check** is consulted only for performance scoring, not for grading your CHOICE/REFLECTION numbers.

## Delta-percent formula
...
before_time_s:  4.80
after_time_s:   3.20
delta_percent: (4.80 − 3.20) / 4.80 × 100 = 33.3 %
```

Sign convention: positive means "improvement". If your "after" is *slower*, you'll see a negative delta — and you should explain what went wrong in REFLECTION.
