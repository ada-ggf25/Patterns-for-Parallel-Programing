...
Linux places a page on the NUMA node of the thread that **first writes** to it — not the thread that calls `new`/`malloc`/`posix_memalign`.
...
The first store to each page is what binds it to a NUMA domain. *Who* does that store determines locality for the rest of the program's lifetime. (Note we use `posix_memalign` not `std::vector<double>(n)` — the constructor would value-initialise from the master and pre-empt our parallel first-touch. Next slide.)

## First-touch — the `std::vector` trap

```cpp
std::vector<double> u(n);            // ← value-initialises every element to 0.0,
                                     //   running on the constructing thread (master).
...
- **The fix** (next slide): allocate with `posix_memalign`, then schedule the first-touch in parallel.

## First-touch — `posix_memalign` + parallel init

{{< include _partials/day4/numa_first_touch__allocate.qmd >}}
...
- 64-byte alignment is the cache-line size on Rome and covers AVX2's 32-byte vector loads — same number we'll reuse for SIMD (later slide).
...
- On Rome with 8 NUMA domains, the `posix_memalign` + parallel-init / parallel-compute pair runs typically **3–5× faster at 128 threads** than the `std::vector` trap two slides back.
...
- **Why not `std::vector<double>(n)`?** Same trap as the first-touch cluster: the constructor value-initialises on the master, *and* gives no alignment guarantee beyond `alignof(double) = 8`. So it's wrong for both reasons.
...
- The same `posix_memalign` you use for first-touch is the right tool for SIMD-friendly memory.
